    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-card text-foreground rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl border border-border animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 sm:p-8 border-b border-border bg-card/50 shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider border border-primary/20 bg-primary/10 text-primary">
                {job.status}
              </span>
            ...
            <h2 className="text-2xl font-black text-foreground">Listing Preview</h2>
          </div>
          <button 
            onClick={onClose}
            className="p-3 text-foreground/60 hover:text-foreground rounded-2xl hover:bg-foreground/10 transition-all active:scale-95"
            aria-label="Close"
          >
            <X size={24} />
          </button>
        ...
        <div className="overflow-y-auto p-6 sm:p-10 flex-1 custom-scrollbar space-y-12">
          {/* Main Title Section */}
          <div>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-foreground leading-tight">{job.title}</h1>
            <p className="text-xl font-bold text-primary mt-2 flex items-center gap-2">
              {job.company}
              <span className="h-1.5 w-1.5 rounded-full bg-foreground/20"></span>
              <span className="text-foreground/60 text-base font-medium">{job.location}</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={item.label} className="p-4 rounded-2xl bg-foreground/[0.03] border border-border/50">
